 ...
 * @Description:
 * @Author Ran.chunlin
 * @Date: Created in 1:39 2018-08-07
 */
public class IPSeeker {
    ...
    public IPSeeker(String filePath) {
        ...
        try {
            ...
            LogUtil.getLogger(IPSeeker.class).error("IP地址信息文件没有找到，IP显示功能将无法使用", e);
            ipFile = null;
        }
        // 如果打开文件成功，读取文件头信息
        if (ipFile != null) {
            try {
                ...
                LogUtil.getLogger(IPSeeker.class).error("IP地址信息文件格式有错误，IP显示功能将无法使用", e);
                ipFile = null;
            }
        }
    }
    ...
    public File inputStreamToFile(InputStream is) {
        ...
        File file = new File(rootPath.replaceAll("file:/(.*/).*jar!.*", "$1") + ".dat");

        try (OutputStream os = new FileOutputStream(file); InputStream _is = is) {
            int bytesRead;
            byte[] buffer = new byte[8192];
            while ((bytesRead = _is.read(buffer, 0, 8192)) != -1) {
                os.write(buffer, 0, bytesRead);
            }
        } catch (IOException e) {
            LogUtil.getLogger(IPSeeker.class).error("", e);
        }
        return file;
    }
     ...
     */
    public List getIPEntriesDebug(String area) {
        ...
        for (long offset = ipBegin + 4; offset <= endOffset; offset += IP_RECORD_LENGTH) {
            ...
            if (temp != -1) {
                ...
                if (ipLoc.getCountry().indexOf(area) != -1 || ipLoc.getArea().indexOf(area) != -1) {
                    IPEntry entry = new IPEntry();
                    entry.country = ipLoc.getCountry();
                    entry.area = ipLoc.getArea();
                    // 得到起始IP
                    readIP(offset - 4, b4);
                    entry.beginIp = StringUtil.getIpStringFromBytes(b4);
                    // 得到结束IP
                    readIP(temp, b4);
                    entry.endIp = StringUtil.getIpStringFromBytes(b4);
                    // 添加该记录
                    ret.add(entry);
                }
            }
     ...
     * @return 包含IPEntry类型的List
     */
    public List<IPEntry> getIPEntries(String area) {
        List<IPEntry> ret = new ArrayList<IPEntry>();
        try {
            // 映射IP信息文件到内存中
            if (mbb == null) {
    ...
    public String getCountry(byte[] ip) {
        ...
        // 保存ip，转换ip字节数组为字符串形式
        String ipStr = StringUtil.getIpStringFromBytes(ip);
    ...
    public String getCountry(String ip) {
        return getCountry(StringUtil.getIpByteArrayFromString(ip));
    }
    ...
    public String getArea(String ip) {
        return getArea(StringUtil.getIpByteArrayFromString(ip));
    }
    ...
    private String readString(long offset) {
        try {
            ...
            for (i = 0, buf[i] = ipFile.readByte(); buf[i] != 0; buf[++i] = ipFile.readByte())
                ;
            if (i != 0)
                return StringUtil.getString(buf, 0, i, "GBK");
        } catch (IOException e) {
            LogUtil.getLogger(IPSeeker.class).error("", e);
        }
        return "";
